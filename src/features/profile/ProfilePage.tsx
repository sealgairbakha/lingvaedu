...
export function ProfilePage() {
  ...
  const certificates = useMemo(() => courses.filter((course) => {
    ...
    const done = new Set(progress.filter((item) => item.userId === user.id && item.courseId === course.id && item.status === "completed").map((item) => item.lessonId));
    return lessonIds.length > 0 && lessonIds.every((id) => done.has(id));
  }), [courses, progress, user]);

  const uploadAvatar = async (file: File) => {
    if (!supabase || !user) return;
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type) || file.size > 5 * 1024 * 1024) { setMessage("Выберите PNG, JPG или WebP размером до 5 МБ."); return; }
  ...
  const save = async () => {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Не удалось сохранить профиль. Попробуйте снова."); }
