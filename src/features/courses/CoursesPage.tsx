...
export function CoursesPage() {
  ...
  const languages = [...new Set(courses.map((x) => x.language))];
  const authors = [...new Set(courses.map((x) => x.author))];
  const visibleCourses = canEditCourses ? courses : courses.filter((c) => c.status === "published" && enrolledCourseIds.includes(c.id));
  ...
  return <main className="content fade coursesWorking">
    ...
    {loading ? <div className="courseEmpty">Загружаем курсы…</div> : rows.length === 0 ? <div className="courseEmpty"><h2>{visibleCourses.length ? "Ничего не найдено" : "Здесь пока нет доступных курсов"}</h2><p>{canEditCourses ? "Создайте первый курс и добавьте в него уроки." : "Курс появится здесь после назначения вам или вашей группе."}</p>{canEditCourses && !courses.length && <button className="btn primary" onClick={create}>Создать курс</button>}</div> : <div className="courseGrid">{rows.map((c) => {
      const isCurrentAuthor = c.authorId === user?.id || (!c.authorId && c.author === displayName);
      const mentorName = isCurrentAuthor ? displayName : c.mentor;
      const mentorAvatar = isCurrentAuthor ? avatarUrl : c.mentorAvatar;
      return <article className="courseCard" key={c.id}><button className={`courseCover ${c.color}`} onClick={() => navigate(`/courses/learn?course=${c.id}`)}><small>{c.language.toUpperCase()}</small></button><div className="courseInfo"><div className="statusRow"><span className={c.status === "published" ? "published" : "draft"}>● {statusLabel[c.status]}</span><small>{new Date(c.updatedAt).toLocaleDateString("ru-RU")}</small></div><h3>{c.title}</h3><p>{c.description}</p><div className="courseStats"><span>▤ {c.modules.reduce((n,m) => n + m.lessons.length, 0)} уроков</span><span>♙ {c.students} учеников</span></div><button className="openCourseBtn" onClick={() => navigate(`/courses/learn?course=${c.id}`)}>{canEditCourses ? "Открыть курс" : "Продолжить обучение"}</button><div className="courseExtras"><div className="mentor"><span>{mentorAvatar ? <img src={mentorAvatar} alt={mentorName}/> : mentorName.slice(0,2).toUpperCase()}</span><div><small>НАСТАВНИК</small><b>{mentorName}</b></div></div>{canEditCourses && <div className="courseActions"><button onClick={() => duplicateCourse(c.id)}>Дублировать</button><button onClick={() => saveCourse({...c,status:c.status === "archived" ? "draft" : "archived"})}>{c.status === "archived" ? "Вернуть" : "В архив"}</button><button className="editCourseAction" onClick={() => edit(c.id)}>Редактировать</button><button className="dangerText" onClick={() => confirm(`Удалить курс «${c.title}»?`) && removeCourse(c.id)}>Удалить</button></div>}</div></div></article>;
    })}</div>}
  </main>;
}
