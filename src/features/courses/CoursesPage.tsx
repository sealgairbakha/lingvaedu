...
export function CoursesPage() {
  ...
  const rows = useMemo(() => courses.filter((c) => (tab === "all" || c.status === tab) && (language === "Все" || c.language === language) && (author === "Все" || c.author === author) && `${c.title} ${c.description}`.toLowerCase().includes(q.toLowerCase())), [courses, tab, language, author, q]);
  ...
  return <main className="content fade coursesWorking">
    ...
    <div className="tabs">{(["all","published","draft","archived"] as const).map((x) => <button key={x} className={tab === x ? "active" : ""} onClick={() => setTab(x)}>{labels[x]} <span>{courses.filter((c) => x === "all" || c.status === x).length}</span></button>)}</div>
    ...
    {loading ? <div className="courseEmpty">Загружаем курсы…</div> : rows.length === 0 ? <div className="courseEmpty"><h2>{courses.length ? "Ничего не найдено" : "Здесь пока нет курсов"}</h2><p>{canEditCourses ? "Создайте первый курс и добавьте в него уроки." : "Курсы появятся после публикации сотрудником."}</p>{canEditCourses && !courses.length && <button className="btn primary" onClick={create}>Создать курс</button>}</div> : <div className="courseGrid">{rows.map((c) => <article className="courseCard" key={c.id}><div className={`courseCover ${c.color}`}><span>{c.code}</span><small>{c.language.toUpperCase()}</small></div><div className="courseInfo"><div className="statusRow"><span className={c.status === "published" ? "published" : "draft"}>● {statusLabel[c.status]}</span><small>{new Date(c.updatedAt).toLocaleDateString("ru-RU")}</small></div><h3>{c.title}</h3><p>{c.description}</p><div className="courseStats"><span>▤ {c.modules.reduce((n,m) => n + m.lessons.length, 0)} уроков</span><span>♙ {c.students} учеников</span></div><div className="mentor"><span>{c.mentor.slice(0,2).toUpperCase()}</span><div><small>НАСТАВНИК</small><b>{c.mentor}</b></div>{canEditCourses && <button onClick={() => edit(c.id)}>Редактировать →</button>}</div>{canEditCourses && <div className="courseActions"><button onClick={() => duplicateCourse(c.id)}>Дублировать</button><button onClick={() => saveCourse({...c,status:c.status === "archived" ? "draft" : "archived"})}>{c.status === "archived" ? "Вернуть" : "В архив"}</button><button className="dangerText" onClick={() => confirm(`Удалить курс «${c.title}»?`) && removeCourse(c.id)}>Удалить</button></div>}</div></article>)}</div>}
  </main>;
}
