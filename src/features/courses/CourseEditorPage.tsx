...
export function CourseEditorPage() {
  ...
  const [course, setCourse] = useState<Course | null>(source || null); const [moduleId,setModuleId]=useState(source?.modules[0]?.id || ""); const [lessonId,setLessonId]=useState(source?.modules[0]?.lessons[0]?.id || ""); const [selected,setSelected]=useState(""); const [saved,setSaved]=useState(true); const [saving,setSaving]=useState(false); const [preview,setPreview]=useState(false);
  useEffect(() => {
  ...
  const save=async()=>{if(course && !saved && !saving){setSaving(true);try{await store.saveCourse(course);setSaved(true);}finally{setSaving(false);}}};
  ...
  return <main className="editorPage courseEditor fade">
    <div className="editorTop"><button className="backBtn" onClick={()=>navigate("/courses")}>←</button><div><input className="courseTitleInput" value={course.title} onChange={e=>mutate(c=>({...c,title:e.target.value}))}/><b>{lesson?.title||"Добавьте урок"}</b></div><select className="publishSelect" value={course.status} onChange={e=>mutate(c=>({...c,status:e.target.value as Course["status"]}))}><option value="draft">Черновик</option><option value="published">Опубликован</option><option value="archived">Архив</option></select><div className="saveState"><i className={saved?"saved":""}/>{saving?"Сохраняем изменения…":saved?"Все изменения сохранены":"Есть несохранённые изменения"}</div><button className="btn ghost" onClick={()=>setPreview(true)}>Предпросмотр</button><button className="btn primary saveCourseButton" disabled={saved || saving} onClick={save}>{saving?"Сохраняем…":saved?"Сохранено":"Сохранить"}</button></div>
